 ...
 */
public final class RCache implements Closeable, Flushable {
...
//      final File directory = new File("./network/cache");
  ...
  public final InternalCache internalCache = new InternalCache() {
    ...
    public Response get(Request request, String key) throws IOException {
      return RCache.this.get(request, key);
    }

    @Override
    public CacheRequest put(Response response, String key) throws IOException {
      return RCache.this.put(response, key);
    }

    @Override
    public void remove(Request request, String key) throws IOException {
      RCache.this.remove(request, key);
    }

    @Override
    public void update(Response cached, Response network) {
      RCache.this.update(cached, network);
    }

    @Override
    public void trackConditionalCacheHit() {
      RCache.this.trackConditionalCacheHit();
    }

    @Override
    public void trackResponse(CacheStrategy cacheStrategy) {
      RCache.this.trackResponse(cacheStrategy);
    }
  };
  ...
  public RCache(File directory, long maxSize) {
    this(directory, maxSize, FileSystem.SYSTEM);
  }

  RCache(File directory, long maxSize, FileSystem fileSystem) {
  ...
  Response get(Request request, String key) {
    //modified by matt
//    String key = key(request.url());
  ...
  CacheRequest put(Response response, String key) {
...
//    }
  ...
  void remove(Request request, String key) throws IOException {
    //modified by matt
...
//          addLenient(varyHeadersBuilder, source.readUtf8LineStrict());
        ...
        varyHeaders = varyHeadersBuilder.build();
          ...
          //modified by matt
          boolean hasRequestBody = source.readInt() == 1;
          if (hasRequestBody) {
              MediaType mediaType = MediaType.parse(source.readUtf8LineStrict());
              long requestBodyContentLength = source.readDecimalLong();
              String requestBodyContent = source.readUtf8LineStrict();
              requestBody = RequestBody.create(mediaType, requestBodyContent);
          } else {
              requestBody = null;
          }
...
//    /**
//     * 反射调用 Headers.Builder#addLenient 方法，解决访问权限问题
//     * @param builder
//     * @param line
//     */
//    void addLenient(Headers.Builder builder, String line) {
////      builder.addLenient(line);
//      if (addLenientMethod == null) {
//        Class clazz = builder.getClass();
//        try {
//          addLenientMethod = clazz.getDeclaredMethod("addLenient", String.class);
//          addLenientMethod.setAccessible(true);
...
//      if (addLenientMethod != null) {
//        try {
//          addLenientMethod.invoke(builder, line);
...
//    }
    ...
    public void writeTo(DiskLruCache.Editor editor) throws IOException {
      ...
      for (int i = 0, size = varyHeaders.size(); i < size; i++) {
        ...
        //modified by matt
      sink.writeInt(requestBody != null ? 1 : 0);
        if (requestBody != null) {
